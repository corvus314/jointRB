...
def relby_conversion(params, waveform_arguments, n_signals = 1):


    param_list =[{} for i in range(n_signals)]
    relby_parameters =[{} for i in range(n_signals)]
    for key, value in params.items():
        name, kind = key.rsplit('_', 1)
        param_list[(ord(kind)-ord('A'))][name]=value
    ...
    for i, parameters in enumerate(param_list):
        ...
        relby_parameters[i] = {'mass_1': parameters['mass_1'], 'mass_2': parameters['mass_2'],
        ...
        relby_parameters[i]['theta_jn'] = xphm.theta_jn_func_op(
            relby_parameters[i]['mass_1'], relby_parameters[i]['mass_2'], relby_parameters[i]['chi_1x'], 
            relby_parameters[i]['chi_1y'], relby_parameters[i]['chi_1z'], relby_parameters[i]['chi_2x'], relby_parameters[i]['chi_2y'], 
            relby_parameters[i]['chi_2z'], relby_parameters[i]['inclination'], relby_parameters[i]['phase'], waveform_arguments['reference_frequency'])

        relby_parameters[i]['kappa']= xphm.kappa_func_op(
            relby_parameters[i]['mass_1'], relby_parameters[i]['mass_2'], relby_parameters[i]['chi_1x'], 
            relby_parameters[i]['chi_1y'], relby_parameters[i]['chi_1z'], relby_parameters[i]['chi_2x'], relby_parameters[i]['chi_2y'], 
            relby_parameters[i]['chi_2z'], relby_parameters[i]['inclination'], relby_parameters[i]['phase'],
            waveform_arguments['reference_frequency'])

    return relby_parameters


def lal_f_max(params, reference_frequency=50.):
    f_max = 0.
    for parameters in params:
        ...
        if chi_eff>0.99:
            fmax_signal = 0.33/(mtotal*msun_sec)
        else:
            fmax_signal = 0.3/(mtotal*msun_sec)
        if fmax_signal > f_max:
            f_max = fmax_signal
    return f_max
...
class OverlappingSignalsRBHOM(Likelihood):
    """
    ...
    def __init__(self, interferometers, fiducial_model, test_model, priors,
                 ...
                 n_signals = 1, simulation=True, delete_strains = False):
        ...
        super(OverlappingSignalsRBHOM, self).__init__(dict())
        ...
        self.fiducial_model = relby_conversion(fiducial_model, self.waveform_arguments, self.n_signals)
        self.mode_array = mode_array
        self.mode_array_np = np.array(self.mode_array)
        self.nifos = len(self.interferometers)
        self.nmodes = len(mode_array)
        self.quad_sum = True
        self.h0, self.hC0 = self.get_binned_detector_response(self.fiducial_model, self.fs)

        self.test_model = relby_conversion(test_model, self.waveform_arguments, self.n_signals)
        self.t0, self.tC0 = self.get_binned_detector_response(self.test_model, self.fs)
        self.data = np.array(
        ...
        self.noise_log_l = self.calculate_noise_log_likelihood()
    ...
    def get_binned_detector_response(self, params, fs):
            
        hL_full = np.zeros([self.nifos, self.nmodes*self.n_signals, len(fs)], dtype=complex)
        Ct_full = np.zeros([self.nifos, self.nmodes*self.n_signals, len(fs)], dtype=complex)

        for i, parameters in enumerate(params):
            Cplus, Ccross, hL = xphm.compute_C_prefactors(parameters, fs, self.waveform_arguments, self.mode_array_np, fs)

            Fplus = np.array([ifo.antenna_response(parameters['ra'], parameters['dec'], 
                parameters['geocent_time'], parameters['psi'], 'plus') for ifo in self.interferometers])
            Fcross = np.array([ifo.antenna_response(parameters['ra'], parameters['dec'], 
                parameters['geocent_time'], parameters['psi'], 'cross') for ifo in self.interferometers])

            dt_geocent = np.array([ifo.time_delay_from_geocenter(
                parameters['ra'], parameters['dec'], parameters['geocent_time']) for ifo in self.interferometers])
            ## this is basically adding time delay from geocent and duration of the signal
            dt_total = dt_geocent+(parameters['geocent_time']-self.interferometers[0].start_time)
            ...
            Ct_full[:, i*self.nmodes:(i+1)*self.nmodes, :] = Cplus_projected+Ccross_projected

            exp_timeshift = np.exp(-1j*2*np.pi*fs*dt_total[:, np.newaxis])
            if hL is not None:
                hL_full[:, i*self.nmodes:(i+1)*self.nmodes, :] = exp_timeshift[:, np.newaxis, :]*hL[np.newaxis, :, :]

        return hL_full, Ct_full 

    def calculate_noise_log_likelihood(self):
        """
    ...
    def log_likelihood_ratio(self):
        ...
        p = relby_conversion(self.parameters, self.waveform_arguments, self.n_signals)

        h, hC = self.get_binned_detector_response(p, fbin)
